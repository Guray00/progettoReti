...
int init(const char* addr, int port){
    ...
    _slog("Avviato socket di ascolto su: %s:%d", addr, port); 
}
...
int signup(){
    ...
    // ricevo utente e password
    ret = recv(csd, buffer, sizeof(buffer), 0);
    if(ret < 0){
        perror("Errore ricezione dati");
        return 0;
    }
    else if(ret == 0){
        _slog("utente disconnesso, registrazione annullata");
        return;
    }

    sscanf(buffer, "%s %s", &usr.username, &usr.pw);
    _slog("Signup ricevuto per utente: %s, pw:%s", usr.username, usr.pw);
    //fflush(stdout);
    ...
    if (ret == 0){
        ...
        if (ret1 && ret2){
            _slog("Utente \"%s\" aggiunto correttamente\n", usr.username);
            fflush(stdout);
            return 1;
        }
        else {
...
int main(int argc, char* argv[]){
    ...
    while(1){
        ...
        _slog("richiesta accettata su porta: %d", ntohs(clientaddr.sin_port));
        fflush(stdout);
        ...
        if (pid == 0){
            ...
            do {
                ...
                switch(request){
                    ...
                    case 0:         // signup
                        _slog("arrivata richiesta di signup");
                        signup();
                        break;

                    case 1:         // in
                        _slog("arrivata richiesta di in");
                        break;
